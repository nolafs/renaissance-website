...
function () {
  function PathEditor(target, vars) {
    ...
    var ctm = this.path.parentNode.getScreenCTM().inverse(),
        size = (ctm.a + ctm.d) / 2 * (vars.handleSize || 4);
...
function () {
  function MotionPathHelper(targetOrTween, vars) {
    ...
    if (!gsap) {
      gsap = vars.gsap || window.gsap || console.warn("GSAP not detected.");
      _win$1 = window;
      _doc$2 = document;
    ...
    animationToScrub = animation.parent.data === "nested" ? animation.parent.parent : animation;
