  ...
  */
  ...
  _getElementBounds = function _getElementBounds(element, context) {
    ...
    if (_windowExists()) {
      ...
      _isAndroid = _win$1.navigator && _win$1.navigator.userAgent.toLowerCase().indexOf("android") !== -1; //Android handles touch events in an odd way and it's virtually impossible to "feature test" so we resort to UA sniffing
      ...
      _addListener(_doc$1, "touchcancel", function () {//some older Android devices intermittently stop dispatching "touchmove" events if we don't listen for "touchcancel" on the document. Very strange indeed.
      });
    ...
    function Draggable(target, vars) {
      ...
      onPress = function onPress(e, force) {
        ...
        if (_touchEventLookup[e.type]) {
          ...
          touchEventTarget = ~e.type.indexOf("touch") ? e.currentTarget || e.target : _doc$1; //pointer-based touches (for Microsoft browsers) don't remain locked to the original target like other browsers, so we must use the document instead. The event type would be "MSPointerDown" or "pointerdown".
          ...
          _addListener(_doc$1, "mousemove", onMove); //attach these to the document instead of the box itself so that if the user's mouse moves too quickly (and off of the box), things still work.
      ...
      onMove = function onMove(e) {
        ...
        if (!hasMoveCallback || _dispatchEvent(self, "move", "onMove") !== false) {
